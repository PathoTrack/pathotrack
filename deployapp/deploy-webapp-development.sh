...
zip -r ../webapp/dist/app.zip ../webapp/dist/* && curl -k -u maulik+pathotrack+development@skarma.com:SamplePass@123 -X PUT -d 'data={"key_pw":"SamplePass@123","keystore_pw":"SamplePass@123"}' https://build.phonegap.com/api/v1/keys/android/$phonegap_development_key_id && curl -k -u maulik+pathotrack+development@skarma.com:SamplePass@123 -X PUT -F file=@../webapp/dist/app.zip https://build.phonegap.com/api/v1/apps/$phonegap_development_app_id
# Create new application version on AWS EB application
aws configure set default.region us-east-1
cd ../api && zip -r pathotrack-$CIRCLE_BUILD_NUM-dev-api.zip . -x './vendor/*' './node_modules/*' && aws s3 cp pathotrack-$CIRCLE_BUILD_NUM-dev-api.zip s3://elasticbeanstalk-us-east-1-751832333305 && aws elasticbeanstalk create-application-version --application-name pathotrack --version-label $CIRCLE_BUILD_NUM-dev --source-bundle S3Bucket=elasticbeanstalk-us-east-1-751832333305,S3Key=pathotrack-$CIRCLE_BUILD_NUM-dev-api.zip && aws elasticbeanstalk update-environment --application-name pathotrack --environment-name pathotrack-dev --version-label $CIRCLE_BUILD_NUM-dev